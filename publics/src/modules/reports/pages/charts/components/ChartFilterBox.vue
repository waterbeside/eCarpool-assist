      ...
      selects_department: [
        {
          key: 0,
          value:
            '<b style="color:#9e88a7;margin-right:2px;" class="fa fa-circle"></b>' +
            this.$t("message.reports.filter.allBranches")
        }
      ],
      max_showType: 1,
      max_department: 3
  ...
  methods: {
    ...
    changeDepartment(data) {
      ...
      if (data.length < 1) {
        ...
        this.$vux.toast.text(this.$t("message.reports.filter.selectAtLeastOne"));
      }
      if (data.length > max) {
        this.scrollTop = this.$refs.filterContent.scrollTop;
        if (data.length > max) {
          this.$vux.toast.text(this.$t('message.reports.filter.selectLimitNum',{num:max}));
    ...
    getDepartmentList() {
      ...
      departmentApi
        .getListByDeep(
          { deep: 3 },
          { cacheKey: "carpool:departmet:selects:deep3", exp: 3600 }
        )
        .then(res => {
          ...
          let newList = [];
          list.map(element => {
            if (this.selects_department_allowids.includes(element.id)) {
              newList.push({
                key: element.id,
                value:
                  '<b style="color:' +
                  cFuns.strToColor(element.fullname) +
                  ';margin-right:2px;" class="fa fa-circle"></b>' +
                  element.name,
                inlineDesc: element.fullname.replace(/\,/g, " / ")
              });
            }
          });
    ...
    onConfirm() {
      if (this.data_department.length < 1) {
        this.$vux.toast.text(this.$t("message.reports.filter.selectAtLeastOneBrannch"));
        return false;
      }
