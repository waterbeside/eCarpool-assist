  ...
  <view-box ref="viewBox" class="cp-viewBox">
    <div class="page-view-content">
      ...
      <div class="alert alert-warning" v-show="!isSuccess">
        <p>
          <b>溢起拼车</b>：
          <b>您现正进行“合并账号”操作</b>，如果你的手机号之前被注册成capool账号，请使用员工账号及其密码和被注册的手机号进行合并操作。合并后手机账号原有的积分将合并到员工号账号。
        </p>
      </div>
      <form
        class="form form-login form-horizontal"
        method="post"
        ...
        <div class="form-inner">
          ...
          <div class="cp-form-group">
            ...
            <input
              ...
              class="form-control form-control-line"
              name="password"
              :placeholder="$t('message.user.passwordOfEmployeeID')"
              v-model="password"
            />
          </div>
          ...
          <div class="cp-form-group">
            ...
            <input
              ...
              id="phone"
              name="phone"
              :placeholder="$t('message.user.phoneNumber')"
              v-model="formData.phone"
              autocomplete="false"
            />
            <button
              class="btn btn-default btn-sendsms"
              :disabled="isCounting"
              @click.prevent="sendCode"
              id="btnSendCode"
            >
              <span v-show="!isCounting">{{$t('message.smsCode.getCode')}}</span>
              <span v-show="isCounting">
                {{$t('message.smsCode.reSend')}} (
                <countdown v-model="countNum" :start="isCounting" @on-finish="finishCount"></countdown>)
              </span>
            </button>
          </div>
          <div class="cp-form-group">
            ...
            <input
              ...
              class="form-control form-control-line"
              name="code"
              :placeholder="$t('message.smsCode.VerificationCode')"
  ...
  methods: {
    ...
    sendCode() {
      ...
      if (!this.formData.phone || !this.formData.phone.trim()) {
        this.$vux.toast.text(
          this.$t("message.user.phone_number_cannot_be_empty")
        );
        return false;
      }
      this.$http
        .post(config.urls.sendSms, postData)
        .then(res => {
          if (res.data.code === 0) {
            ...
            this.$vux.toast.text(
              this.$t("message.user.phone_number_not_need_merge")
            );
          } else {
    ...
    doSubmit() {
      ...
      this.isLoading = true;
      this.$http
        .post(config.urls.checkSmsCode, postData)
        .then(res => {
          if (res.data.code === 0) {
...
.form-group .form-control-line {
  ...
  &:focus {
    box-shadow: none;
    border: none;
    background-image: linear-gradient(#6342a1, #6342a1),
      linear-gradient(#6342a1, #6342a1);
  }
  // &:active { }
}
...
.cp-form-group {
  ...
  label.control-label {
    ...
    font-size: 16px;
    i.fa {
      color: #aaa; /* color:rgb(230, 136, 127)*/
    }
    svg {
      fill: #aaa;
      height: 21px;
