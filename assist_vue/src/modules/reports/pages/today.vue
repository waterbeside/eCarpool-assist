<template>
  <view-box id="Page-today">
    <div class="page-view-content">
      <h1 class="cp-title">
        {{$t("message.reports.TodayCarpool")}}
...
export default {
  ...
  computed: {
    ...
    columns() {
      return [
        {
          field: "carnumber",
          title: this.$t("message.licensePlate"),
          width: 100,
          titleAlign: "center",
          columnAlign: "center"
        },
        ...
        },
        {
          ...
          width: this.field_width_2,
          titleAlign: "center",
          columnAlign: "center"
        },
        {
          field: "p_name",
          title: this.$t("message.passenger"),
          width: this.field_width,
          titleAlign: "center",
          columnAlign: "center"
        },
  ...
  methods: {
    ...
    getList() {
      ...
      this.$http
        .get(config.urls.reports.getTodayInfo)
        .then(res => {
          this.isLoading = false;
          if (res.data.code === 0) {
            let data = res.data.data;
            data.lists.forEach(el => {
               el.format_time = dateFormat(new Date(el.time * 1000), "YYYY-MM-DD HH:mm");
               el.d_branch = (el.d_department.split(','))[0];
               el.p_branch = (el.p_department.split(','))[0];
              return el;
            });
